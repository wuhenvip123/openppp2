...
PppApplication::PppApplication() noexcept
{
    ...
    if (NULL != hConsole)
    {
        COORD cSize = { 120, 42 }; /* ppp::win32::Win32Native::IsWindows11OrLaterVersion(); */
        ...
        else
        {
            context->post(
                [context]() noexcept
                {
                    ...
                    auto timeout = Timer::Timeout(context, 1000, 
                        []() noexcept
                        {
...
int main(int argc, const char* argv[]) noexcept
{
    ...
    return Executors::Run(APP->GetBufferAllocator(), /* std::bind(&PppApplication::Main, PPP_APPLICATION_DEFAULT_APP_DOMAIN, std::placeholders::_1, std::placeholders::_2); */
        [APP, prepared_status](int argc, const char* argv[]) noexcept -> int
        {
            ...
            {
                fprintf(stdout, "[%s]%s\r\n", chnroutes2_gettime(chnroutes2_gettime()).data(), ppp::win32::Win32Native::OptimizationSystemNetworkSettings() ? "OK" : "FAIL");
                return -1;
            }
#endif
